...
import { ShippingLabelDocument } from "@/components/admin/ShippingLabelDocument";
...
export const Route = createFileRoute("/admin/orders/$orderId/label")({
  ...
  head: () => ({
    meta: [{ title: "Shipping Label — MakeMyThing.in Admin" }, { name: "robots", content: "noindex" }],
  }),
  ...
  useEffect(() => {
    ...
    if (search.download === "1") {
      const timer = window.setTimeout(() => {
        downloadElementAsPdf("shipping-label-document", `${order.order_number}-label.pdf`, "label").catch(
    ...
    <div className="min-h-screen bg-gray-100 p-4">
      <div className="no-print mx-auto mb-4 flex max-w-md flex-wrap justify-center gap-2">
        <Button onClick={() => printDocument()} className="rounded-full">
          <Printer className="mr-2 h-4 w-4" /> Print Label (4×6)
        </Button>
        <Button
          variant="secondary"
      ...
      <ShippingLabelDocument order={order} />
    </div>
  );
}
